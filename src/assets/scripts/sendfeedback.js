        ...
        fetch('http://localhost:3004/sendReview/' + id,
            ...
            .then((data) => {
                ...
                for (index = 0; index < questions.length; index++) {
                    document.getElementById("repeat-emotions1").innerHTML += '<fieldset class="emotions" id="emotions' + index + '"> <legend><b>'+questions[index].toString().substring(0,1).toUpperCase()+questions[index].toString().substring(1)+'</b></legend><div class="emotion-section"> <label for="emotion' + index + '">Choose an emotion<label><br> <select name="emotion' + index + '" id="emotion' + index + '"> <option value="happy">Happy</option><option value="surprised">Surprised</option><option value="sad">Sad</option><option value="fearful">Fearful</option>  <option value="discusted">Discusted</option>  <option value="angry">Angry</option> <option value="appreciation">Apreciation</option>  <option value="expectant">Expectant</option> </select></div> <div class="intensity-section">  <label for="intensity">Choose an intensity</label> <form oninput="x.value=getIntensityNames(parseInt(intensity.value),' + index + ');">   <input type="range" value="1" max="2" id="intensity" name="intensity"><br><br>  <output name="x" for="intensity"> <p>  </p></output>  </form> </div> </fieldset> </div><br><br>';
                }
            }
            );
